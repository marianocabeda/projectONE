...
// CrearPersona inserta una nueva persona en la base de datos dentro de una transacción
// y devuelve el ID de la nueva fila.
func CrearPersona(ctx context.Context, tx *sql.Tx, p *modelos.Persona) (int64, error) {
	query := `
        INSERT INTO persona (nombre, apellido, sexo, cuil, fecha_nacimiento, id_direccion, 
                             distrito_nombre, departamento_nombre, provincia_nombre, 
                             telefono, telefono_alternativo, email, id_tipo_iva, id_usuario_creador)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
	...
	res, err := stmt.ExecContext(ctx,
		p.Nombre, p.Apellido, p.Sexo, p.Cuil, p.FechaNacimiento, p.IDDireccion,
		p.DistritoNombre, p.DepartamentoNombre, p.ProvinciaNombre,
		p.Telefono, p.TelefonoAlternativo, p.Email, p.IDTipoIva, p.IDUsuarioCreador,
	)
	if err != nil {
