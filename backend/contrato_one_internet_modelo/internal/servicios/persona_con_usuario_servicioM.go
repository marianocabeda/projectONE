...
func (s *UsuarioService) ResendVerificationToken(ctx context.Context, email string) (string, time.Time, error) {
	...
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		logger.Error.Printf("No se pudo iniciar transacción en ResendVerificationToken: %v", err)
		return "", time.Time{}, err
	}
	defer tx.Rollback()

	// Buscar usuario por email que no esté borrado
	var idUsuario int
	var emailVerificado bool
	var requiereVerif bool
	err = tx.QueryRowContext(ctx, `
		SELECT id_usuario, email_verificado, requiere_verificacion
		FROM usuario
		WHERE email = ? AND borrado IS NULL
		LIMIT 1
		`, email).Scan(&idUsuario, &emailVerificado, &requiereVerif)
	if err != nil {
	...
	logger.Debug.Printf("usuarioID=%d, emailVerificado=%v, requiereVerif=%v", idUsuario, emailVerificado, requiereVerif)

	// Si ya está verificado o no requiere verificación, no crear token
	if emailVerificado || !requiereVerif {
		logger.Debug.Printf("No se genera token. emailVerificado=%v, requiereVerif=%v", emailVerificado, requiereVerif)
		return "", time.Time{}, nil
	}

	// Invalidar tokens previos no usados para este usuario (asegurar un token activo)
	...
	tokenStr, err := utilidades.GenerarTokenSeguro(utilidades.TokenSize32)
	if err != nil {
		logger.Error.Printf("Error al generar nuevo token de verificación: %v", err)
		return "", time.Time{}, err
	}
	...
	logger.Debug.Printf("Token generado para usuario %d: %s, expiración: %v", idUsuario, tokenStr, expiracion)

	if err := tokenRepo.CrearEmailVerificacionToken(ctx, tokenRecord); err != nil {
		logger.Error.Printf("No se pudo insertar token: %v", err)
		return "", time.Time{}, err
	}

	if err := tx.Commit(); err != nil {
		logger.Error.Printf("No se pudo hacer commit en ResendVerificationToken: %v", err)
		return "", time.Time{}, err
	}
